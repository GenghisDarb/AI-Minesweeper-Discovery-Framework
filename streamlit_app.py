...
from ai_minesweeper.torus_recursion.dpp14_recursion_engine import DPP14RecursionEngine
    ...
    if st.session_state.revealed_hypotheses:
        ...
        for cell in st.session_state.revealed_hypotheses:
            label = getattr(cell, 'description', 'Hypothesis')
            st.sidebar.write(label)

    if st.session_state.board:
        render_unresolved_hypotheses(st.session_state.board)
        render_revealed_hypotheses_summary(st.session_state.board)

    # Confidence display
    if st.session_state.beta_confidence:
        conf = st.session_state.beta_confidence.mean() if hasattr(st.session_state.beta_confidence, 'mean') else st.session_state.beta_confidence.get_confidence()
        display_confidence(conf, mode="streamlit")
...
=======
"""
Streamlit web application for AI Minesweeper with χ-recursive form.

This application provides:
- Step-by-step control and auto-discovery modes
- Real-time confidence display and χ-cycle visualization
- TORUS theory alignment visualization
- Interactive board with click-to-reveal/flag functionality
"""
...
# Configure page
st.set_page_config(
    page_title="AI Minesweeper - χ-Recursive Form",
    page_icon="💣",
    layout="wide",
...
from src.ai_minesweeper.board import Board, CellState
from src.ai_minesweeper.constraint_solver import ConstraintSolver
from src.ai_minesweeper.ui_widgets import MinesweeperVisualizer, TooltipManager, AccessibilityHelper


class StreamlitMinesweeperApp:
    """
    Streamlit application for AI Minesweeper with χ-recursive capabilities.
    """
